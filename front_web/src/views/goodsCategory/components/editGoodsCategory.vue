...
<script>
    import { getcategoryinfo,getParentCategoryList,editcategory } from "@/api/goodsCategory.js";
    export default{
        data() {
            ...
            return {
                ...
                rules: {
                    ...
                    parent_category_id: [{required: false, trigger: "change", validator: selectempty}],
                    seller_id: [{required: true, trigger: "blur", validator: number}],
                    status: [{required: true, trigger: "change", validator: selectempty}],
                    sort: [{required: true, trigger: "blur", validator: number}],
                    category_img: [{required: true, trigger: "blur", validator: imgempty}]
                },
            }
        },
        components:{
...
//            }
        ...
        methods: {
            ...
            getInfo() {
                const _this = this;
//                console.log(this.category_id)
                getcategoryinfo(this.category_id).then(res => {
//                    console.log(res.data.data)
                    var result = res.data.data;
                    _this.form.category_name = result.category_name;
                    _this.form.parent_category_id = result.parent_category_id;
                    _this.form.seller_id = result.seller_id;
                    _this.form.status = result.status;
                    _this.form.sort = result.sort;
                    _this.form.category_img = result.category_img;
                }).catch(err => {
                    console.log(error)
                });
            },
...
//                    console.log(valid)
                    if(valid){
                        editcategory(_this.category_id,this.form.category_name,this.form.parent_category_id,this.form.seller_id,this.form.category_img,this.form.status,this.form.sort).then(res => {
                            console.log(res)
            ...
            //渲染菜单
            parentCategory(){
                getParentCategoryList(0,this.seller_id).then(res => {
