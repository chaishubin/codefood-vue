<template>
  <div id="editCategory">
    <el-dialog width="80%" top="5vh" :before-close="BcloseDialog" title="编辑分类" :visible.sync="dialogFormVisible" :close-on-click-modal="false"
      :lock-scroll="true">
...
<script>
    import { getcategoryparentlist,editmodify,getcategorydetail } from "@/api/productCategories.js"
    export default{
        data() {
        ...
        methods: {
            ...
            getInfo() {
                const _this = this; 
                getcategorydetail(this.id).then(res => {
                    console.log(res)
                    if(res.data.status == '200'){
                        _this.form.name = res.data.data.name;
                        _this.form.parent_id = res.data.data.parent_id;
                        _this.form.sort = res.data.data.sort;
                        _this.form.status = res.data.data.status;
                        _this.form.icon = res.data.data.icon;
                        _this.form.icon_show = _this.baseurl + res.data.data.icon;
                    }else{
                        this.$message({
                            showClose: true,
            ...
            submitform() {
              ...
              this.$refs.editForm.validate(valid => {
                  console.log(valid)
                  if(valid){
                      editmodify(this.id,this.form.name,this.form.parent_id,this.form.sort,this.form.status,this.form.icon).then(res => {
                          console.log(res)
                          if(res.data.status == '200'){
                              this.$message({
                                  showClose: true,
